	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				...
				'installing'                      => __( 'Installing Plugin: %s', 'FoodRecipe' ),
				'oops'                            => __( 'Something went wrong.', 'FoodRecipe' ),
				...
				'dashboard'                       => __( 'Return to the dashboard', 'FoodRecipe' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'FoodRecipe' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'FoodRecipe' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'FoodRecipe' ),
				'dismiss'                         => __( 'Dismiss this notice', 'FoodRecipe' ),
			);
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'FoodRecipe' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */
					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'FoodRecipe' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'FoodRecipe' ) . '</a>'  : '';

					/** Define all of the action links */
					$action_links = apply_filters(
						'FoodRecipe_notice_action_links',
						array(
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'FoodRecipe' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'FoodRecipe' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'FoodRecipe' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'FoodRecipe' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'FoodRecipe' ) : __( 'Recommended', 'FoodRecipe' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'FoodRecipe' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'FoodRecipe' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'FoodRecipe' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'FoodRecipe' ),
				'source' => __( 'Source', 'FoodRecipe' ),
				'type'   => __( 'Type', 'FoodRecipe' ),
				'status' => __( 'Status', 'FoodRecipe' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'FoodRecipe' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'FoodRecipe' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'FoodRecipe' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'FoodRecipe' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'FoodRecipe' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'FoodRecipe' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'FoodRecipe' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'FoodRecipe' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'FoodRecipe' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'FoodRecipe' );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'FoodRecipe' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'FoodRecipe' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'FoodRecipe' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'FoodRecipe' ) . '</span><span class="hidden">' . __( 'Hide Details', 'FoodRecipe' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'FoodRecipe' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'FoodRecipe' );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'FoodRecipe' );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'FoodRecipe' );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'FoodRecipe' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'FoodRecipe' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'FoodRecipe' ) . '</span><span class="hidden">' . __( 'Hide Details', 'FoodRecipe' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'FoodRecipe' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'FoodRecipe' );
				}
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'FoodRecipe' ) . '">' . __( 'Return to the Dashboard', 'FoodRecipe' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
